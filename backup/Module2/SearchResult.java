...
public class SearchResult {
    ...
    public String getTitleofResult() throws InterruptedException {
        ...
        WebElement yonexTitleName = parentElement.findElement(By.tagName("p"));
    ...
    public Boolean validateSizeChartContents(List<String> expectedTableHeaders,
        ...
        try {
             ...
             * same order
             */
            // WebElement tableElement = driver.findElement(By.xpath("//div[@class
            // ='MuiDialog-container MuiDialog-scrollPaper css-ekeie0']/div"));
            ...
            for (WebElement headerElements : tableHeaders) {
                ...
                // int rowCount = driver.findElements(By.xpath("//table[@class = 'MuiTable-root
                // css-1v2fgo1']//tr")).size();
                // int colCount = driver.findElements(By.xpath("//table[@class = 'MuiTable-root
                // css-1v2fgo1']//th")).size();
                // for(int i = 1; i < rowCount; i++){
                // for (int j = 1; i <= colCount; j++){
                // tempBodyRowElements = driver.findElement(By.xpath("//table[@class =
                // 'MuiTable-root css-1v2fgo1']/tbody/tr["+(i+1)+"]/td["+j+"]")).getText();
                    ...
                    for (int j = 0; j < expectedTableBody.get(i).size(); j++) {
                        ...
                        if (!expectedTableBody.get(i).get(j).equals(tempBodyRowElements)) {
                            System.out.println("Failure in Body Comparison: Expected:  "
                                    + expectedTableBody.get(i).get(j) + " Actual: "
                                    + tempBodyRowElements);
                            status = false;
                        }
                    }
